...
def main(wav, onnx, shapes, precision, max_workspace_size):
    time_start = time()
    engine = load_engine(onnx, shapes, precision, max_workspace_size)
    ...
    time_end = time()
    print(f"Mel spectrogram computed in {time_end - time_start:.2f} seconds")

    time_start = time()
    f0 = ParselMouthPitchExtractor(f0_min=40.0, f0_max=2000.0, keep_zeros=False)(audio, 44100, pad_to=mel.shape[-1])
    time_end = time()
    print(f"F0 computed in {time_end - time_start:.2f} seconds")

    # 检查 mel 和 f0 的长度是否超出 shapes 中的 maxshape
    if mel.shape[1] > shapes["mel"][2][1]:
        raise ValueError(f"mel length {mel.shape[1]} exceeds maximum length {shapes['mel'][2][1]}")
    if f0.shape[0] > shapes["f0"][2][1]:
        raise ValueError(f"f0 length {f0.shape[0]} exceeds maximum length {shapes['f0'][2][1]}")
    ...
    time_start = time()
    inputs = {"mel": c.numpy(), "f0": f0.numpy()}
    outputs = trt_inference.infer(inputs)
    output = outputs['waveform'].flatten()
    time_end = time()
    print(f"Inference done in {time_end - time_start:.2f} seconds")
...
if __name__ == '__main__':
    ...
    input_onnx = "nsf_hifigan_44.1k_hop512_128bin_2024.onnx"
    max_workspace_size = 6 << 30  # 6 GB
    precision = "fp32"

    shapes = { # (min, opt, max)
    "mel": ((1, 128, 128), (1, 1500, 128), (1, 3000, 128)),
    "f0": ((1, 128), (1, 1500), (1, 3000))
}
    main(input_wav, input_onnx, shapes, precision, max_workspace_size)